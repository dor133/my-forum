...
import { useDeleteCommentMutation, useAddCommentLikeMutation, useRemoveCommentLikeMutation } from '../../../store/rtk/comments'
...
export function Comment(commentData: CommentQueryData) {
    ...
    const [removeCommentLike] = useRemoveCommentLikeMutation()

    const [commentLiked, setCommentLiked] = useState(commentData?.likes.length === 0 ? false : true)

    const handleClickedCommentLike = () => {
        if (commentLiked) {
            setCommentLiked(false)
            removeCommentLike({ id: commentData._id, postId: id })
        } else {
            setCommentLiked(true)
            addCommentLike({ id: commentData._id, postId: id })
        }
    }
    ...
    return (
        <div className="border-t border-gray-200">
            ...
            <div className="flex justify-between">
                ...
                {payload ? (
                    commentLiked ? (
                        <button onClick={handleClickedCommentLike}>
